...
import dev.bpavuk.gemini.models.codeExec.CodeExecutionResult
import dev.bpavuk.gemini.models.codeExec.ExecutableCode
import dev.bpavuk.gemini.models.tools.ExpectedFunctionResult
import dev.bpavuk.gemini.models.tools.FunctionCall
import dev.bpavuk.gemini.models.tools.FunctionResponse
...
public sealed interface Part {
    ...
    public data class FunctionResponseSurrogate<T : ExpectedFunctionResult>(val functionResponse: FunctionResponse<T>) :
        Part
    ...
    public companion object {
        public fun Blob(mimeType: String, data: String): BlobSurrogate = BlobSurrogate(
            dev.bpavuk.gemini.models.Blob(
                mimeType,
                data
            )
        )

        public fun <T : ExpectedFunctionResult> FunctionCall(name: String, args: T): FunctionCallSurrogate<T> =
            FunctionCallSurrogate(
                dev.bpavuk.gemini.models.tools.FunctionCall(name, args)
            )

        public fun <T : ExpectedFunctionResult> FunctionResponse(name: String, args: T): FunctionResponseSurrogate<T> =
            FunctionResponseSurrogate(
                dev.bpavuk.gemini.models.tools.FunctionResponse(name, args)
            )

        public fun FileData(mimeType: String, fileUri: String): FileDataSurrogate = FileDataSurrogate(
            dev.bpavuk.gemini.models.FileData(mimeType, fileUri)
        )

        public fun ExecutableCode(language: Language, code: String): ExecutableCodeSurrogate = ExecutableCodeSurrogate(
            dev.bpavuk.gemini.models.codeExec.ExecutableCode(language, code)
        )

        public fun CodeExecutionResult(outcome: Outcome, result: String): CodeExecutionResultSurrogate =
            CodeExecutionResultSurrogate(
                dev.bpavuk.gemini.models.codeExec.CodeExecutionResult(outcome, result)
            )
    }
}
